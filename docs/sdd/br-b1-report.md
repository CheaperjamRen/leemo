...
- **网关模式 env 无真 key**：遍历 env 全值断言 `not.toContain(relay2.apiKey)`（fixture 特意给 relay2 带 `test-key-relay-SHOULD-NEVER-LEAK-...` 使之为真泄漏测试）。
...
- **新增** `tests/bridge/fixtures/providers.ts`（Provider fixtures，`test-key-…` 假值）
...
- 无真 key 形状串：仅 `test-key-…` 假 sentinel（简报明许）；广义 `(sk|xai|gsk|glm|Bearer)[-_]{12,}` 扫描 src/bridge+tests/bridge 只命中这些假值。
