...
1. **占位 token 路由 + 真 key 只在上游头**（参数化 Authorization / x-api-key 两 header）：断言 mock 上游收到 `Bearer sk-test-…`、上游收到真 model `deepseek-chat`、客户端响应是 `type:"message"` 且不含真 key、网关日志不含真 key。
