...
- **无真实 key 形状字符串**：对全部三个新源文件 + 三个新测试文件 + 扩展的 fixture 文件跑 `grep -nE "sk-[A-Za-z0-9]{16,}|Bearer [A-Za-z0-9_\-]{20,}"`，零命中（测试里的 key 均为 `test-key-...` 字面假值，`Bearer` 只出现在拼接表达式 `` `Bearer ${apiKey}` `` 里，非字面泄漏）。
