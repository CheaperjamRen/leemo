...
- **监督体制：Comate 为常设监督者**（只读核验，产物在 `comate/`），验收以其清单为准
- 进行中：**启动轮 1 步 1 = Electron 承重墙**，交接=`docs/handoffs/NEXT-SESSION-electron-step1.md`（含技术底牌速查+Comate 验收标准）。通电竖切已落盘（代码 409f116 / 文档 d9b2aba，591 tests 绿）
- 后续队列：步 2 SQLite 持久化 → momo 人格 prompt 组装器卡（06 §7.2 五件套，文本用户亲审）→ 工作台 InputArea 统一/Batch 7 收尾
- 旧线暂挂：网关 G2-G4（G4 需向用户要 RELAY2_* 配置）
